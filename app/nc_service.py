...
def obter_ou_criar_causas(cliente, nomes_causas: list[str], usuario_id: str) -> list[int]:
    ids = []
...
def calcular_reincidencia(cliente, causa_ids: list[int], nc_id_excluir: int | None = None) -> str:
    if not causa_ids:
        return "Não"

    resultado = cliente.table("nc_causas").select("nc_id").in_("causa_id", causa_ids).execute()
...
def _montar_saida_nc(cliente, nc: dict) -> dict:
    ...
    nc["causas"] = [linha["causas"]["descricao"] for linha in causas_rel.data if linha.get("causas")]
    return nc
...
def listar_ncs(usuario: UsuarioLogado) -> list[dict]:
    ...
    cliente = cliente_do_usuario(usuario.token)
    resultado = (
        cliente.table("nao_conformidades").select("*").order("criado_em", desc=True).execute()
    )
...
def buscar_nc(usuario: UsuarioLogado, nc_id: int) -> dict:
    ...
    if not resultado.data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="NC não encontrada.")
    return _montar_saida_nc(cliente, resultado.data[0])
...
def atualizar_nc(usuario: UsuarioLogado, nc_id: int, dados) -> dict:
    ...
    atualizada = cliente.table("nao_conformidades").update(payload).eq("id", nc_id).execute()
...
def avaliar_nc(usuario: UsuarioLogado, nc_id: int, dados) -> dict:
    ...
    if dados.decisao not in ("validar", "invalidar"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="decisao deve ser 'validar' ou 'invalidar'.")

    if dados.decisao == "invalidar" and not (dados.motivo_invalidacao and dados.motivo_invalidacao.strip()):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Informe o motivo da invalidação.")
    ...
    _registrar_historico(nc_id, usuario.id, "aberta", payload["status"], dados.motivo_invalidacao)
...
def enviar_nc(usuario: UsuarioLogado, nc_id: int) -> dict:
    ...
    _registrar_historico(nc_id, usuario.id, "validada", "aguardando_analise", "NC enviada ao colaborador e supervisor")
...
def aplicar_feedback(usuario: UsuarioLogado, nc_id: int, dados) -> dict:
    ...
    if not dados.feedback or not dados.feedback.strip():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Informe o feedback.")
    ...
    _registrar_historico(nc_id, usuario.id, "aguardando_analise", "aguardando_aceite", "Feedback aplicado")
...
def aceitar_nc(usuario: UsuarioLogado, nc_id: int, dados) -> dict:
    ...
    if not atualizada.data:
        raise HTTPException(
            ...
            detail="Não foi possível registrar o aceite (a NC pode não ser sua ou não estar aguardando aceite).",
        )

    _registrar_historico(nc_id, usuario.id, "aguardando_aceite", "concluida", "Aceite formal do colaborador")

    return _montar_saida_nc(cliente, atualizada.data[0])


def _registrar_historico(nc_id: int, usuario_id: str, status_anterior: str | None, status_novo: str, observacao: str | None):
